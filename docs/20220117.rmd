...
ggplot(res, aes(x=lambda, y=z1_mean)) +
  ...
  geom_abline(slope=1, intercept=0)
```
So this does work but it stops working when $V_{P,l}$ starts to get a bit higher than 0.1. What about the variance of the expected data variable
...
for some set of variants that have a distribution of allele frequencies. Need to choose some arbitrary value $\sigma^2_b$ that will lead to desired $V_A$.

## Example

Let's simulate some beta values for fitness, according to some expected model and that give rise to a specified Va. Then estimate the effects using a linear model on the number of children (where number of children is sampled using poisson or sample methods).
...
```{r}
tibble(agreement=c(out$bhat1, out$bhat2), what=rep(c("poisson", "sample"), each=nrow(out))) %>%
  ggplot(., aes(y=agreement, x=what)) +
  geom_boxplot()
```
...
Ok I'm not getting a problem here, the effect sizes are small enough even in the 2 SNP case?
...
- Estimating genetic effects on observed data using a linear model in a polygenic context approximates simulated effects on fitness *provided that beta is divided by the mean number of children* - this has not been done
