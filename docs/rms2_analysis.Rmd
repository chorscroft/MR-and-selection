...
```{r}
x$mvmr(x$gwashits$rsid[hit], traitlist=x$rsid_scan[[x$gwashits$rsid[hit]]]$id[-c(77,105)])
...
```{r}
x$mvmr(x$gwashits$rsid[hit], traitlist=x$rsid_scan[[x$gwashits$rsid[hit]]]$id[-5])
x$mv_dat[[x$gwashits$rsid[hit]]][["result"]][[1]] %>% dplyr::select(exposure, b, se, pval)
```
